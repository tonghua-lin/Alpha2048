...
<p align="center">
  <img src="docs/assets/gameplay-demo.gif" width="640" alt="Alpha2048 游戏演示">
</p>

## 100 局表现

**上一代模型在 depth 1 下跑满 100 局：均分 121,442，56% 的对局达到 8192；相比历史 mixed4 基线，均分提高 57.7%。**

| 模型 | 平均分 | 中位数 | 达到 4096 | 达到 8192 |
|---|---:|---:|---:|---:|
| 上一代学习模型 | 121,442 | 130,266 | 99% | 56% |
| 历史 mixed4 基线 | 77,020 | 78,414 | 80% | 18% |

这里展示上一代模型的完整 100 局记录，因为当前发布模型只有 19 局阶段评估。两组结果使用不同随机种子，mixed4 基线与早期训练语料有重叠，因此这是行为表现对比，不是独立留出测试或同种子配对实验。完整配置与统计见 [100 局评估记录](docs/benchmark-100-games.md)。
...
AI 助手以十字布局显示四个方向的相对推荐度，支持 depth 0 / 1 / 2、单步执行和自动运行。CPU 可直接使用；源码环境也可在兼容的 NVIDIA 显卡与 CUDA 版 PyTorch 上选择 GPU。
