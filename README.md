...
Prebuilt CPU packages, when published, are available from [Releases](https://github.com/tonghua-lin/Alpha2048/releases). Download and extract the entire ZIP, then open:

- `2048.exe` — game and game AI.
- `ScreenAssistant.exe` — external-board recommendations.

Keep the `_internal` directory beside both executables. The CPU build needs no Python or CUDA installation; its GPU option reports unavailable. Builds are unsigned. Do not upload the ZIP to Git history; attach it to a GitHub Release instead.
...
Run these commands from the repository root. The repository assets must remain alongside the source; a standalone Python wheel is not the desktop distribution format. For manual play without AI/OCR, installing `-e .` is enough. The game loads its AI runtime only when requested.
...
## Build the CPU release

After setting up `.venv`, double-click **Build CPU.cmd**. It creates a separate CPU build environment, exports inference weights, freezes both entry points and runs executable self-tests before making the ZIP. The development environment is unchanged. See [build instructions](docs/build-cpu.md).

The first build requires network access and can take several minutes. Build logs are under `reports/packaging`; runtime logs are under `%LOCALAPPDATA%/Alpha2048/logs`. Neither is committed.
...
- `packaging/`: portable-build scripts and third-party notices.
- `docs/`: usage, algorithm and build documentation.
